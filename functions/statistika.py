...
def all_satatistikas(topic_id,question_id,topic_name,question_name, id, from_date, end_date, page, limit, db, status):
    statistikas = db.query(Statistika).filter(Statistika.id >= 0)

    if topic_id:
        statistikas=statistikas.filter(Statistika.topic_id==topic_id)

    if question_id:
        statistikas=statistikas.filter(Statistika.question_id==question_id)
    if topic_name:
...
def add_statistikas(form, db):
    ...
    db.add(new_statistika)
    db.commit()
    db.refresh(new_statistika)
    statistika_adding(topic_id=form.topic_id, question_id=form.question_id,topic_name=form.topic_name, question_name=form.question_name, answer_a=form.answer_a, answer_b=form.answer_b, answer_c=form.answer_c, answer_d=form.answer_d, answer_e=form.answer_e, answer_f=form.answer_f, db=db)
...
def update_statistikas(id, form, db):
    ...
    db.query(Statistika).filter(Statistika.id == id).update({
        Statistika.topic_id:form.topic_id,
        Statistika.question_id:form.question_id,
